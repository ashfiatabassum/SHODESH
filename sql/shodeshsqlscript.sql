...
CREATE INDIVIDUAL (
  ...
  first_name VARCHAR(30) CHECK (first_name REGEXP '^[A-Za-z ]+$'),
  last_name VARCHAR(30) CHECK (last_name REGEXP '^[A-Za-z ]+$'),
  username VARCHAR(15),
...
CREATE TABLE FOUNDATION(
  ...
  bkash VARCHAR(11) CHECK (bkash REGEXP '^0[0-9]{10}$'),
  bank_account VARCHAR(18),
  status ENUM('varified', 'unvarified') DEFAULT 'unvarified',
...
CREATE TABLE EVENT_CREATION(
  ...
  division VARCHAR(30) CHECK (division REGEXP '^[A-Za-z ]+$'),
  doc BLOB,
  status ENUM('varified', 'unvarified') DEFAULT 'unvarified',
