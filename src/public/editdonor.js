...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!form) {
    console.error("❌ editDonorForm not found in DOM");
    return;
  }
  ...
  form.addEventListener("submit", async function (e) {
    ...
    if (!donorId) {
      showMessage("Donor ID missing. Please sign in again.", "error");
      return;
    }
    ...
    if (document.getElementById("updateUsername").checked) {
      ...
      const checkRes = await fetch(`/api/donor/check-username`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ field: "username", value: updates.username, donorId })
      });
...
console.log("➡️ Endpoint:", `/api/donor/update/${donorId}`);
    ...
    try {
      ...
      console.log("📡 PUT request sent to /api/donor/update/", donorId);
