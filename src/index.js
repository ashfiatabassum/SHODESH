...
// Event creation routes must be registered after all other specific routes
// to prevent intercepting paths like /api/staff/signup
app.use("/api/eventcreation", eventcreationroutes);
...
app.get("/api/debug/routes", (req, res) => {
  ...
  app._router.stack.forEach((mw) => {
    if (mw.route && mw.route.path) {
      const methods = Object.keys(mw.route.methods).filter(
        (m) => mw.route.methods[m]
      );
      routes.push({ path: mw.route.path, methods });
    } else if (mw.name === "router" && mw.handle && mw.handle.stack) {
      mw.handle.stack.forEach((r) => {
        if (r.route && r.route.path) {
          const methods = Object.keys(r.route.methods).filter(
            (m) => r.route.methods[m]
          );
          ...
          const composed =
            mw && mw.regexp && mw.regexp.fast_slash
              ? r.route.path
              : prefix
              ? `${prefix}${r.route.path}`
              : r.route.path;
...
app.post("/donate", async (req, res) => {
  ...
  if (!donorId)
    return res
      ...
      .json({
        success: false,
        message: "You must sign in as a donor before donating.",
      });
  if (
    !creation_id ||
    typeof creation_id !== "string" ||
    creation_id.length !== 7
  )
    return res
      .status(400)
      .json({ success: false, message: "Valid creation_id is required." });
  ...
  try {
    await db
      .promise()
      .query("CALL sp_record_donation(?,?,?)", [creation_id, donorId, amount]);
  } catch (err) {
    ...
    if (err && err.sqlState === "45000") {
      return res
        ...
        .json({
          success: false,
          message: err.message || "Donation rejected by business rule",
        });
    }
    return res
      .status(500)
      .json({ success: false, message: "Database error", detail: err.message });
  }
  ...
  try {
    ...
    const meta =
      rows && rows[0] ? rows[0] : { remaining: null, eligible: null };
    return res.json({
      success: true,
      message: "Donation recorded successfully",
      remaining: meta.remaining,
      stillEligible: !!meta.eligible,
    });
  } catch (err2) {
...
app.get("/api/events/:id/eligibility", async (req, res) => {
  ...
  if (!id || id.length !== 7)
    return res
      .status(400)
      .json({ success: false, message: "Valid creation_id required" });
  try {
    const [rows] = await db
      .promise()
      .query(
